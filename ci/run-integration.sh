...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-recommender
...
# ATTACH (`${VGI_RECOMMENDER_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run recommender_worker.py`) — the
...
#                        `uv run --python 3.13 <repo>/recommender_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/recommender_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_RECOMMENDER_WORKER (the LOCATION) and, for the
...
case "$TRANSPORT" in
  subprocess)
    export VGI_RECOMMENDER_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    # extra (waitress); WORKER_CMD must resolve it — CI passes
    # `--extra http` and the PEP 723 header lists the extra too.
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/recommender-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/recommender-http-server.log"
    ...
    export VGI_RECOMMENDER_WORKER="http://127.0.0.1:$PORT"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/recommender-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/recommender-unix-server.log"
    ...
    export VGI_RECOMMENDER_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_RECOMMENDER_WORKER) ..."
