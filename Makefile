# vgi-recommender — dev and test targets.
#
# Usage:
...
#
# test-sql is self-contained: it points VGI_RECOMMENDER_WORKER at the worker run
...
WORKER_STDIO    ?= uv run --python 3.13 recommender_worker.py
...
test-sql:
	PATH="$(HAYBARN_BIN):$$PATH" \
		VGI_RECOMMENDER_WORKER="$(WORKER_STDIO)" \
		haybarn-unittest --test-dir "$(TEST_DIR)" "$(TEST_PATTERN)"

lint:
	uv run ruff format --check .
	uv run ruff check .
	uv run mypy vgi_recommender/
